...
void accessData (
    ...
    while ( curr_line != NULL ) {
	   // printf("Current Line Address:%lld", curr_line->tag);
    ...
    /* ... */
    cache_line_t *add = calloc(b,sizeof(cache_line_t));
    add->tag = tag;
    if(cache->front == NULL){
	cache->front = add;
    	cache->back = add;
    }else{
    	cache->back->next_cache_line = add;
	cache->back = add;
    }
    cache->occupancy++;
    //free(add);
