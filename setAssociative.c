...
void accessData (
    ...
    /* ... */
	mem_addr_t tag_index = addr >> (s+b);
	mem_addr_t set_index = (addr >> b) & (S - 1);

    // Find the correct cache set
    /* ... */
	cache_set_t set_cache = cache[set_index];

    // Cache hit
    /* ... */
	for(int i = 0; i < E; i++){
		if(set_cache[i].valid && set_cache[i].tag == tag_index){
			set_cache[i].recent_use = program_counter;
			(*hit_count)++;
			return;
		}
	}
    ...
    /* ... */
	int placement = 0;
	int set_min = set_cache[placement].recent_use;
	for(int i = 1; i < E; i++){
		if(set_cache[i].recent_use < set_min){
			placement = i;
			set_min = set_cache[i].recent_use;
		}
	}

    // If cache set line already in use, then record an eviction
    /* ... */
	if(set_cache[placement].valid == true){
		(*eviction_count)++;
	}

	set_cache[placement].valid = true;
	set_cache[placement].recent_use = program_counter;
	set_cache[placement].tag = tag_index;
   /* ... */
}
