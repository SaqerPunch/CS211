...
void quicksort(int len, int array[len]){
	if(len == 1){
		return;	
	} else { 
		int pivot_index = partition (len,  array);
		quicksort(pivot_index+1, array);
		quicksort(len-(pivot_index+1), array+pivot_index+1);
	}
}

int quickselect(int len, int array[len],int kth){
	quicksort (len, array);
	return array[kth];
}
